...
module.exports = {
  ...
  Open: APEv2,
  delete: _delete,
  is_valid_apev2_key: is_valid_apev2_key,
  APEValue: APEValue,
  APEBinaryValue: APEBinaryValue,
  APETextValue: APETextValue,
  APEExtValue: APEExtValue,
  TEXT: TEXT,
  BINARY: BINARY,
  EXTERNAL: EXTERNAL,
};
...
var HAS_HEADER = (1 << 31) >>> 0;
var HAS_NO_FOOTER = (1 << 30) >>> 0;
var IS_HEADER  = (1 << 29) >>> 0;

// TODO - delete this?
//class error(IOError): pass

util.inherits(APENoHeaderError, AbstractError);
function APENoHeaderError() {
  AbstractError.apply(this, arguments);
}

util.inherits(APEUnsupportedVersionError, AbstractError);
function APEUnsupportedVersionError() {
  AbstractError.apply(this, arguments);
}

util.inherits(APEBadItemError, AbstractError);
function APEBadItemError() {
  AbstractError.apply(this, arguments);
}
...
_APEv2Data.prototype.__find_metadata = function(fileobj) {
  ...
  if (buf.toString('binary', 0, 8) === 'APETAGEX') {
    this.fileobj_seek -= 8;
  ...
  if (buf.toString('binary', 0, 3) === "TAG") {
...
_APEv2Data.prototype.__fix_brokenness = function(fileobj) {
  ...
  while (start > 0) {
    ...
    this.fileobj_seek += fs.readSync(fileobj, buf, 0, 8, this.fileobj_seek);
    if (buf.toString('binary') === "APETAGEX") {
      this.fileobj_seek -= 8;
...
APEv2.prototype.__parse_tag = function(tag, count) {
  ...
  for (var i = 0; i < count; ++i) {
    ...
    var value = tag.toString('binary', offset, offset += 1);
    var key = value;
    while (key[key.length - 1] !== '\x00' && value) {
      value = tag.toString('binary', offset, offset += 1);
      key += value;
    }
    if (key[key.length - 1] === "\x00") {
      key = key.substring(0, key.length - 1);
    }
    value = tag.toString('binary', offset, offset += size);
...
APEv2.prototype.getItem = function(key) {
  ...
  if (lowerKey in this.__dict) {
    return this.__dict[lowerKey];
  } else {
...
APEv2.prototype.setItem = function(key, value) {
  ...
  if (! (value instanceof _APEValue)) {
    ...
    if (typeof value === 'string') {
      ...
      value = new APEValue(value.join("\0"), TEXT);
...
APEv2.prototype.save = function(filename) {
  ...
  var offset = 0;
  header.write("APETAGEX", offset, 8, 'binary'); offset += 8;
  ...
  header.writeUInt32LE((HAS_HEADER|IS_HEADER) >>> 0, offset); offset += 4;
  ...
  // version, tag size, item count, flags
  offset = 0;
  footer.write("APETAGEX", offset, 8, 'binary'); offset += 8;
...
function _delete(filename) {
  ...
  try {
    ...
    if (err.name !== 'APENoHeaderError') throw err;
  }
}
...
_APEValue.prototype._internal = function(key) {
  ...
  var offset = 0;
  buffer.writeUInt32LE(this.value.length, offset); offset += 4;
  buffer.writeUInt32LE((this.kind << 1) >>> 0, offset); offset += 4;
...
APEv2File.score = function(filename, fileobj, header) {
  ...
  return (footer.toString('binary').indexOf("APETAGEX") >= 0) -
    (header.toString('binary', 0, 3) === 'ID3');
};
